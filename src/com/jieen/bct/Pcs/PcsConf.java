package com.jieen.bct.Pcs;

public class PcsConf {
	
	//public final static String APIKEY = "iG2ffdkYaq8kIjrSfvjMcUrf";
	public final static String APIKEY="pap3bYWoPQQatqoyiaVUBrd4";	//My Key
	//social demo
	public final static String SINA_APP_KEY = "your app key";		//‘›Œ¥ π”√
    
    //personal file storage demo
    //public final static String PERSON_STORAGE_DIR_NAME = "/apps/FrontiaDevDemo/pic";
    public final static String PERSON_STORAGE_DIR_NAME = "/apps/CloudTerminal";
    public final static String PERSON_STORAGE_DIR_NAME1 = "/";

    public final static String PERSON_STORAGE_FILE_NAME = "/apps/CloudTerminal/pic/custom.jpg";
	
    public final static String LOCAL_FILE_NAME = "/sdcard/Download/custom.jpg";
    public final static String FILE1 = "/sdcard/Download/custom.jpg";
    public final static String FILE2 = "/sdcard/Download/custom.jpg";
    public final static String FILE3 = "/sdcard/Download/custom.jpg";
    
    //app file storage demo
    public final static String APP_STORAGE_FILE_NAME = "custom.jpg";

    //statistics demo
    public static final String eventId = "1";//Ã·–—
    public static final String reportId = "fd4278e8f6";
}
